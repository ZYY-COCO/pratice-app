    ...
    <MentorPageHeader title="我的咨询" @back="goBack">
      <template #right>
        <button
          ...
          @tap="loadOrders"
        >
          <text class="my-consultations-refresh-glyph" aria-hidden="true">↻</text>
        </button>
      </template>
    </MentorPageHeader>
    ...
    <scroll-view scroll-y class="my-consultations-scroll" @scrolltolower="loadMoreOrders">
      <view class="my-consultations-content">
        <view class="my-consultations-list-head">
          <strong>我发起的咨询</strong>
          <text>{{ recordCountText }}</text>
        </view>

        <view v-if="loading && !orders.length" class="my-consultations-state">正在同步咨询记录…</view>
        <view v-else-if="error" class="my-consultations-state error">
          <text>{{ error }}</text>
          <button @tap="loadOrders">重新加载</button>
        </view>
        <view v-else-if="!orders.length" class="my-consultations-empty">
          <view class="my-consultations-empty-icon">咨</view>
          <strong>还没有咨询记录</strong>
          <text>完成一次前辈咨询后，聊天记录会自动保存在这里。</text>
...
.my-consultations-list-head strong {
  color: var(--consultation-ink);
}
