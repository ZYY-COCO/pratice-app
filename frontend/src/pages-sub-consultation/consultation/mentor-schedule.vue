<template>
  <view class="mentor-schedule-page" :style="themeInlineStyle">
    <MentorPageHeader title="预约时段" @back="goBack" />

    <scroll-view scroll-y class="mentor-schedule-scroll" @scrolltolower="loadMoreScheduleSlots">
      <view class="mentor-schedule-content">
        ...
        <view class="mentor-schedule-card mentor-schedule-list-card">
          ...
          <view v-else class="mentor-schedule-slot-list">
            ...
            <view class="mentor-schedule-load-state" @tap="loadMoreScheduleSlots">
              {{ scheduleLoadingMore ? '正在加载更多…' : scheduleHasMore ? '继续下滑加载更多时段' : '已加载全部未来时段' }}
            </view>
          </view>
        </view>
...
const scheduleNextCursor = ref('')
const scheduleHasMore = ref(false)
...
async function loadSchedule() {
  if (scheduleLoading.value || scheduleLoadingMore.value) return
  scheduleLoading.value = true
  scheduleNextCursor.value = ''
  scheduleHasMore.value = false
  try {
    const [profilePayload, slotsPayload] = await Promise.all([
      fetchMyMentorProfile(),
      fetchMyMentorAvailabilitySlots({ limit: 30 })
    ])
    ...
    scheduleNextCursor.value = String(slotsPayload?.next_cursor || '')
    scheduleHasMore.value = slotsPayload?.has_more === true
  } catch (error) {
...
async function loadMoreScheduleSlots() {
  if (scheduleLoading.value || scheduleLoadingMore.value || !scheduleHasMore.value || !scheduleNextCursor.value) return
  scheduleLoadingMore.value = true
  try {
    const payload = await fetchMyMentorAvailabilitySlots({ limit: 30, cursor: scheduleNextCursor.value })
    const nextSlots = normalizeAvailabilitySlots(payload?.items)
    const merged = [...availabilitySlots.value]
    nextSlots.forEach((slot) => {
      const index = merged.findIndex((item) => item.id === slot.id)
      if (index >= 0) merged[index] = slot
      else merged.push(slot)
    })
    availabilitySlots.value = sortAvailabilitySlots(merged)
    scheduleNextCursor.value = String(payload?.next_cursor || '')
    scheduleHasMore.value = payload?.has_more === true
  } catch (error) {
    uni.showToast({ title: error?.detail || '更多预约时段加载失败', icon: 'none' })
  } finally {
    scheduleLoadingMore.value = false
  }
}
...
function goBack() {
  uni.navigateBack({
    fail() {
      uni.reLaunch({ url: '/pages-sub-consultation/consultation/mentor-apply?mode=center' })
    }
  })
}
</script>
...
.mentor-schedule-add-button { position: relative; box-sizing: border-box; width: 100%; height: 76rpx; min-height: 76rpx; margin: 26rpx 0 0; padding: 0 20rpx; border: 0; border-radius: 20rpx; background: #3478f6; color: #fff; display: flex; align-items: center; justify-content: center; text-align: center; font-size: 24rpx; line-height: 1; font-weight: 900; white-space: nowrap; box-shadow: 0 10rpx 22rpx rgba(52, 120, 246, .2); }.mentor-schedule-add-button[disabled] { height: 76rpx; min-height: 76rpx; padding-top: 0; padding-bottom: 0; background: #bdcadc; box-shadow: none; }
.mentor-schedule-add-button::after,
.mentor-schedule-refresh::after,
.mentor-schedule-slot-side button::after { border: 0; }
.mentor-schedule-refresh { position: relative; box-sizing: border-box; width: 112rpx; min-width: 112rpx; height: 52rpx; min-height: 52rpx; margin: 0; padding: 0 12rpx; border: 0; border-radius: 15rpx; background: #edf4ff; color: #4e73aa; display: flex; align-items: center; justify-content: center; text-align: center; font-size: 20rpx; line-height: 1; font-weight: 850; white-space: nowrap; }
...
.mentor-schedule-load-state { padding: 22rpx 0 4rpx; color: #8a9ab0; text-align: center; font-size: 18rpx; line-height: 1.4; font-weight: 750; }
...
.mentor-schedule-slot-side button { position: relative; box-sizing: border-box; width: 132rpx; min-width: 132rpx; height: 48rpx; min-height: 48rpx; margin: 0; padding: 0 10rpx; border: 2rpx solid #d7e5f8; border-radius: 14rpx; background: #f7faff; color: #5f7faa; display: flex; align-items: center; justify-content: center; text-align: center; font-size: 19rpx; line-height: 1; font-weight: 850; white-space: nowrap; }
.mentor-schedule-slot-side button.reopen { border-color: #bfd7ff; background: #edf4ff; color: #3478f6; }
.mentor-schedule-add-button[loading]::before { position: absolute; top: 0; bottom: 0; left: 18rpx; width: 26rpx; height: 26rpx; margin: auto 0; }
.mentor-schedule-refresh[loading]::before,
.mentor-schedule-slot-side button[loading]::before { position: absolute; top: 0; bottom: 0; left: 6rpx; width: 20rpx; height: 20rpx; margin: auto 0; }

.mentor-schedule-page { background: var(--gyt-page-bg); }
.mentor-schedule-card { border-color: var(--gyt-primary-border, #d9e7fc); background: var(--gyt-panel-bg, #ffffff); box-shadow: 0 14rpx 34rpx var(--gyt-primary-shadow, rgba(52, 120, 246, 0.06)); }
.mentor-schedule-date-option,.mentor-schedule-time-slot { border-color: var(--gyt-primary-border, #dce8fa); background: var(--gyt-primary-tint, #fbfdff); }
.mentor-schedule-date-option.selected,.mentor-schedule-time-slot.selected { border-color: var(--gyt-primary, #3478f6); background: var(--gyt-primary-soft, #edf4ff); color: var(--gyt-primary, #3478f6); box-shadow: 0 8rpx 18rpx var(--gyt-primary-shadow, rgba(52, 120, 246, 0.1)); }
.mentor-schedule-date-option.selected view { color: var(--gyt-primary, #3478f6); }
.mentor-schedule-add-button { background: var(--gyt-primary-gradient, #3478f6); box-shadow: 0 10rpx 22rpx var(--gyt-primary-shadow, rgba(52, 120, 246, 0.2)); }
.mentor-schedule-refresh,.mentor-schedule-slot-status,.mentor-schedule-slot-side button.reopen { border-color: var(--gyt-primary-border, #bfd7ff); background: var(--gyt-primary-soft, #edf4ff); color: var(--gyt-primary, #3478f6); }
.mentor-schedule-bottom-space { height: calc(56rpx + env(safe-area-inset-bottom)); }
@media (max-width: 350px) { .mentor-schedule-content { padding-right: 18rpx; padding-left: 18rpx; }.mentor-schedule-card { padding: 22rpx; }.mentor-schedule-slot-item { gap: 12rpx; }.mentor-schedule-slot-side button { width: 124rpx; min-width: 124rpx; padding: 0 8rpx; font-size: 18rpx; } }
</style>
