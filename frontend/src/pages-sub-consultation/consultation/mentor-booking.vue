<template>
  <view class="mentor-booking-page" :style="themeInlineStyle">
    ...
    <scroll-view scroll-y class="mentor-booking-scroll">
      <view v-if="mentor" class="mentor-booking-content">
        ...
        <view class="mentor-booking-heading">
          ...
          <view class="mentor-booking-window">{{ mentor.consultationWindowMinutes || 60 }}分钟</view>
        </view>
        ...
        <view class="mentor-booking-notice">
          <text>预约说明</text>
          <view>付款后无需再次等待接单；预约时间开始后，由认证前辈在站内发起本次 {{ mentor.consultationWindowMinutes || 60 }} 分钟文字咨询。</view>
        </view>
      </view>
...
function nextStep() {
  ...
  uni.navigateTo({
    url: `/pages-sub-consultation/consultation/mentor-consult-form?mentorId=${encodeURIComponent(mentor.value.id)}&mode=booking`
  })
}
...
.mentor-booking-hero,.mentor-booking-date-card,.mentor-booking-notice { border-color: var(--gyt-primary-border, #d9e7fc); background: var(--gyt-panel-bg, #ffffff); box-shadow: 0 14rpx 34rpx var(--gyt-primary-shadow, rgba(52, 120, 246, 0.07)); }
.mentor-booking-avatar.tone-blue,.mentor-booking-name-row > text:last-child,.mentor-booking-window { background: var(--gyt-primary-soft, #edf4ff); color: var(--gyt-primary, #3478f6); }
.mentor-booking-slot { border-color: var(--gyt-primary-border, #cfe0fb); background: var(--gyt-primary-tint, #fbfdff); }
.mentor-booking-slot view,.mentor-booking-slot.selected { color: var(--gyt-primary, #3478f6); }
.mentor-booking-slot.selected { border-color: var(--gyt-primary, #3478f6); background: var(--gyt-primary-soft, #edf4ff); box-shadow: 0 8rpx 18rpx var(--gyt-primary-shadow, rgba(52, 120, 246, 0.12)); }
.mentor-booking-empty-slots { border-color: var(--gyt-primary-border, #d7e4f6); background: var(--gyt-primary-tint, #ffffff); }
.mentor-booking-footer { border-color: var(--gyt-primary-border, #dbe7f8); background: var(--gyt-primary-tint, #ffffff); }
.mentor-booking-footer button { background: var(--gyt-primary-gradient, #3478f6); box-shadow: 0 10rpx 22rpx var(--gyt-primary-shadow, rgba(52, 120, 246, 0.2)); }
</style>
