<template>
  <view class="mentor-chat-page">
    ...
    <view v-if="mentor" class="mentor-chat-service-tip" :class="{ ended: consultationEnded }">
      ...
      <view>{{ consultationEnded ? '聊天记录已保留' : '服务时间：60分钟' }}</view>
    </view>
    ...
    <scroll-view scroll-y class="mentor-chat-scroll" :scroll-into-view="scrollTarget" scroll-with-animation>
      <view v-if="mentor" class="mentor-chat-content">
        <view class="mentor-chat-system-message">前辈已接受咨询，本次60分钟咨询窗口已开始。</view>

        <view class="mentor-chat-context-card">
          <view class="mentor-chat-context-title">我的情况</view>
        ...
        <view class="mentor-chat-message-row mentor" id="mentor-welcome">
          <view class="mentor-chat-avatar" :class="`tone-${mentor.avatarTone || 'blue'}`">{{ mentor.avatar }}</view>
          <view class="mentor-chat-message-stack">
            <view class="mentor-chat-sender">{{ mentor.maskedName }}</view>
            <view class="mentor-chat-bubble">你好，我已经看到你的咨询信息了。我们可以先从目标院校和当前复习节奏开始梳理。</view>
          </view>
        </view>
        ...
        <view
          ...
          :class="message.sender === 'mentor' ? 'mentor' : 'user'"
        >
          <view v-if="message.sender === 'mentor'" class="mentor-chat-avatar" :class="`tone-${mentor.avatarTone || 'blue'}`">{{ mentor.avatar }}</view>
          <view class="mentor-chat-message-stack">
            <view v-if="message.sender === 'mentor'" class="mentor-chat-sender">{{ mentor.maskedName }}</view>
    ...
    <view v-if="mentor && consultationEnded" class="mentor-chat-completed-bar">
      <view><strong>本次咨询已结束</strong><text>感谢你的信任，聊天记录已保留。</text></view>
      <button @tap="openReview">评价本次咨询</button>
    ...
    <view v-else-if="mentor" class="mentor-chat-input-bar">
      ...
      <input v-model="messageInput" placeholder="输入想继续咨询的问题" placeholder-class="mentor-chat-placeholder" confirm-type="send" @confirm="sendText" />
      ...
      <button class="mentor-chat-finish" @tap="finishConsultation">结束</button>
    </view>

    <view v-if="reviewVisible" class="mentor-review-mask" @tap="closeReview">
      <view class="mentor-review-sheet" @tap.stop>
        ...
        <button class="mentor-review-submit" @tap="submitReview">提交评价</button>
      </view>
    </view>
  </view>
</template>
...
const remainingServiceSeconds = ref(3156)
const scrollTarget = ref('mentor-welcome')
...
onLoad((options) => {
  ...
  if (draft?.mentorId === mentor.value?.id) {
    ...
    messages.value = Array.isArray(draft.messages) ? draft.messages : []
    consultationEnded.value = draft.orderStatus === 'completed'
  }
...
onBeforeUnmount(stopServiceTimer)

function startServiceTimer() {
  stopServiceTimer()
  serviceTimer = setInterval(() => {
    if (remainingServiceSeconds.value <= 1) {
      finishConsultation()
      return
    }
    remainingServiceSeconds.value -= 1
  }, 1000)
}
...
function sendText() {
  ...
  if (!content || consultationEnded.value) return
  const draft = appendConsultationMessage({ sender: 'user', type: 'text', content })
  messages.value = Array.isArray(draft?.messages) ? draft.messages : messages.value
  messageInput.value = ''
  scrollToBottom()
  setTimeout(() => {
    if (consultationEnded.value) return
    const response = appendConsultationMessage({
      sender: 'mentor',
      type: 'text',
      content: '收到。你可以先把目前每周的复习安排和最担心的环节告诉我，我会结合你的目标院校一起拆解。'
    })
    messages.value = Array.isArray(response?.messages) ? response.messages : messages.value
    scrollToBottom()
  }, 480)
}
...
function sendVoice() {
  if (consultationEnded.value) return
  const draft = appendConsultationMessage({ sender: 'user', type: 'voice', duration: '00:12' })
  messages.value = Array.isArray(draft?.messages) ? draft.messages : messages.value
  uni.showToast({ title: '已发送一条语音消息（演示）', icon: 'none' })
  scrollToBottom()
}

function sendImage() {
  if (consultationEnded.value) return
  const draft = appendConsultationMessage({ sender: 'user', type: 'image' })
  messages.value = Array.isArray(draft?.messages) ? draft.messages : messages.value
  uni.showToast({ title: '已添加咨询图片（演示）', icon: 'none' })
  scrollToBottom()
}
...
function finishConsultation() {
  ...
  setConsultationOrderStatus('completed')
  nextTick(scrollToBottom)
}
...
function showMore() {
  uni.showActionSheet({ itemList: ['查看咨询规则', '结束本次咨询'], success: ({ tapIndex }) => { if (tapIndex === 1) finishConsultation() } })
}

function openReview() { reviewVisible.value = true }
function closeReview() { reviewVisible.value = false }
