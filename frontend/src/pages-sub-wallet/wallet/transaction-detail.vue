  ...
  <view class="wallet-detail-page" :style="themeInlineStyle">
    ...
    <scroll-view scroll-y class="wallet-detail-scroll">
      <view v-if="transaction" class="wallet-detail-content">
        ...
        <view class="wallet-detail-footnote">{{ transaction.fundMode === 'demo' ? '当前交易来自 Demo 隔离账本，不计入真实资金。' : '当前交易来自服务端真实账本。' }}</view>
      </view>

      <view v-else class="wallet-detail-missing">
        <view class="wallet-detail-missing-icon">账</view>
        <strong>未找到这笔账单</strong>
        <text>该账单缓存可能已失效，请返回钱包刷新服务端账本后重试。</text>
...
import { fetchWalletTransaction, getCachedWalletTransactionById } from '../../api/wallet'
import {
  walletStatusLabels,
  walletTypeLabels
...
onLoad((options) => {
  ...
  transaction.value = getCachedWalletTransactionById(options?.id, role.value)
  void loadTransaction(options?.id)
})
...
async function loadTransaction(id) {
  ...
  try {
    transaction.value = await fetchWalletTransaction(transactionId, {
      role: role.value,
      ...(import.meta.env.DEV ? { mode: 'demo' } : {})
    })
  } catch (error) {
    if (!transaction.value) {
      uni.showToast({ title: error?.detail || '账单详情加载失败', icon: 'none' })
    }
  }
}
...
function goBack() {
  uni.navigateBack({
    fail() {
      uni.reLaunch({ url: `/pages-sub-wallet/wallet/index?role=${role.value}` })
    }
  })
}
</script>
...
.wallet-detail-hero,.wallet-detail-card { border-color: var(--gyt-primary-border, #dce8fb); background: var(--gyt-panel-bg, #ffffff); box-shadow: 0 18rpx 42rpx var(--gyt-primary-shadow, rgba(52, 120, 246, 0.09)); }
.wallet-detail-status,.wallet-detail-missing-icon { background: var(--gyt-primary-soft, #edf4ff); color: var(--gyt-primary, #3478f6); }
</style>
