<template>
  <view class="admin-page" :class="{ 'question-mode': activeTab === 'questions' }" :style="themeInlineStyle">
    <AppPageHeader :title="pageTitle" :subtitle="pageSubtitle" fixed @back="goBack">
      <template #right>
        <button v-if="activeTab === 'questions'" class="hero-refresh-btn" aria-label="题库操作" @tap="questionCreateMode ? openCreateQuestionActions() : refreshQuestionManager">
          <image v-if="questionCreateMode" class="hero-action-icon" src="/static/ui-icons/more.svg" mode="aspectFit" />
          <AppRefreshIcon v-else />
        </button>
      </template>
    </AppPageHeader>
...
async function bootstrap() {
  if (!isLoggedIn()) {
    uni.redirectTo({ url: `/pages/login/index?redirect=${encodeURIComponent('/pages-sub-admin/admin/index')}` })
    return
  }
...
function openQuestionImageImport() {
  clearQuestionSelection()
  uni.navigateTo({ url: '/pages-sub-admin/admin/question-image-import' })
}
...
.hero-refresh-btn :deep(.app-refresh-icon),
.hero-action-icon { width: 34rpx; height: 34rpx; display: block; }
